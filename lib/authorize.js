...
class Authenticator {
	...
	startAuthorization() {
		console.log( 'Now authenticating. Opening in your browser...' );
		const params = {
	...
	handleRequest( req, res ) {
		...
		res.writeHead( 200 );
		res.end( this.options.render( 'Authenticated! You can now close this window and go back to hm-cli.' ) );
	...
	const options = Object.assign(
		{
			...
			secret: null,
			port: 4101,
			render: message => `<!doctype html><html><body><p>${ message }</p></body></html>`,
		},
		opts
	);
