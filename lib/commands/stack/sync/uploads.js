	...
	try {
		...
		if ( ! backup && ! logId ) {
			...
			try {
				...
				console.log( chalk.dim( `Resume later with: altis-cli stack sync uploads ${ app } --resume ${ logId }` ) );
			} catch ( err ) {
...
export default {
	command: 'uploads <app>',
	description: 'Sync uploads from a remote Vantage app into local-server.',
	builder: cmd => {
