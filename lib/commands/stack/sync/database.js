...
const handler = async function ( argv ) {
	...
	if ( ! yes && ! latest ) {
		...
		if ( backup ) {
			const age = formatAge( new Date( backup.date ) );
			confirmMsg = `Import backup ${ chalk.bold( backup.id ) } (${ chalk.dim( age + ' old' ) }) and replace local DB? This cannot be undone.`;
		} else if ( logId ) {
			confirmMsg = `Resume backup ${ chalk.bold( logId ) } and import into local DB? This cannot be undone.`;
		} else {
			confirmMsg = `Create a new backup of ${ chalk.bold( app ) } and replace local DB? This cannot be undone.`;
		}
	...
	try {
		...
		if ( ! backup && ! logId ) {
			...
			if ( siteId && siteId.length ) {
				...
				if ( siteIdList.includes( '1' ) ) {
					console.log( chalk.yellow( 'Note: site ID 1 is the main site — its tables use the base prefix, syncing all tables.' ) );
					tableList = [];
				} else {
			...
			try {
				...
				console.log( chalk.dim( `Resume later with: altis-cli app sync-local database ${ app } --resume ${ logId }` ) );
			} catch ( err ) {
...
export default {
	command: 'database <app>',
	description: 'Sync the database from a remote Altis Dashboard app into local-server.',
	builder: cmd => {
