...
export async function confirm( message ) {
	...
	if ( ! ok ) {
		throw new UserCancelled( 'Cancelled.' );
	}
}
...
export async function downloadArchive( url, dest ) {
	...
	// Backup URLs are pre-signed S3 URLs — plain fetch, no auth headers.
	...
	const progress = progressStream( { length: size, time: 200 } );
...
export async function importUploads( localPath ) {
	const hasS3 = await new Promise( resolve => {
		const proc = spawn( 'composer', [ 'server', 's3', '--help' ], {
			cwd: localPath,
			stdio: 'pipe',
		} );
	...
	await runComposerServer( localPath, hasS3 ? [ 's3', 'import-uploads' ] : [ 'import-uploads' ] );
}

export async function runPostSync( localPath ) {
	const registered = await new Promise( resolve => {
		const proc = spawn( 'composer', [ 'server', 'cli', '--', 'cli', 'has-command', 'altis post-sync' ], {
			cwd: localPath,
			stdio: 'pipe',
		} );
		proc.on( 'close', code => resolve( code === 0 ) );
...
// --- Resolve table names for multisite site IDs ---

export async function resolveTablesForSiteIds( v, app, siteIds ) {
	const resp = await v.fetch( `stack/applications/${ app }/database-tables` );
	if ( ! resp.ok ) {
		throw new Error( `Failed to fetch table list: ${ resp.status }` );
	}
	const data = await resp.json();
	const tables = Array.isArray( data ) ? data : ( data.tables || [] );

	// Derive the table prefix from the blogs table (e.g. "wp_blogs" → prefix "wp_")
	let prefix = '';
	for ( const name of tables ) {
		const m = name.match( /^(.+_)blogs$/ );
		if ( m ) {
			prefix = m[1];
			break;
		}
	...
	if ( ! prefix ) {
		throw new Error( 'Could not determine table prefix — is this a multisite install?' );
	}

	const result = [];
	for ( const id of siteIds ) {
		const pattern = `${ prefix }${ id }_`;
		result.push( ...tables.filter( t => t.startsWith( pattern ) ) );
	}
	return result;
}
...
export function addCommonOptions( cmd ) {
	...
	cmd.option( 'latest', {
		description: 'Use the latest existing backup without prompting. Implies --yes.',
		type: 'boolean',
		default: false,
	} );
	cmd.option( 'resume', {
...
export function addDatabaseOptions( cmd ) {
	cmd.option( 'table', {
		description: 'Table to include in the partial backup. Repeatable; also accepts comma-separated values.',
		type: 'array',
	} );
	cmd.option( 'site-id', {
		description: 'Multisite site ID to include in the partial backup. Repeatable; also accepts comma-separated values.',
		type: 'array',
	} );
...
export function addUploadsOptions( cmd ) {
	...
	cmd.option( 'site-id', {
		description: 'Multisite site ID to sync uploads for. Maps to sites/{id}/ prefix.',
		type: 'array',
	} );
}
