	...
	async removeExistingWP() {
		switch ( this.status.type ) {
			...
			case TYPE_VIP:
				...
				const dirMap = {
					"plugins": path.join( contentPath, 'plugins' ),
					"themes": path.join( contentPath, 'themes' ),
					"client-mu-plugins": path.join( contentPath, 'client-mu-plugins' ),
					"languages": path.join( contentPath, 'languages' ),
				};
				const dirKeys = Object.keys( dirMap );
				for ( let i = 0; i < dirKeys.length; i++ ) {
					const dirName = dirKeys[0];
					const fullPath = path.join( this.root, dirName );
					await this.renamePath( fullPath, dirMap[ dirName ] );
				}
				break;
