...
class Converter {
	...
	async setUpComposer() {
		...
		if ( fs.existsSync( composerJsonPath ) ) {
			...
			console.log( '  - Setting config.platform' );
	...
	async addAltis() {
		...
		console.log( ' --' );
		const res = child_process.spawnSync( 'composer', [ 'require', '--with-all-dependencies', 'altis/altis:~16' ], {
			cwd: this.root,
			stdio: 'inherit',
			// shell: true,
		} );
		...
		console.log( ' --' );
		const res2 = child_process.spawnSync( 'composer', [ 'require', '--dev', '--with-all-dependencies', 'altis/local-server:~16' ], {
			cwd: this.root,
			stdio: 'inherit',
			// shell: true,
		} );
