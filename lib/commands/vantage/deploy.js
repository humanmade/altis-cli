...
module.exports = function ( argv ) {
	const { debug, force, region } = argv;
	const proxyRegion = 'ap-southeast-2';

	const v = new Vantage( region, proxyRegion );
	let stack;
	if ( argv.stack ) {
		stack = argv.stack;
	}
	...
	function streamLog( log ) {
		...
		console.log( chalk.yellow( `  hm vantage deploy --stack ${stack} --resume ${log}\n` ) );
