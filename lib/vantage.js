...
function getStacks( config ) {
	return Promise.all( REGIONS.map( region => {
		const cached = config.cache.get( `stacks:${ region }` );
		if ( cached && Date.now() < cached.timestamp + STACK_CACHE_LIFETIME ) {
			return Promise.resolve( [ region, cached.stacks ] );
		}
		...
		return vantage.fetch( 'stack/applications' )
			...
			.then( stacks => {
				config.cache.set( `stacks:${ region }`, {
					timestamp: Date.now(),
					stacks
				} );
		...
		const regionMap = {};
		regions.forEach( ( [ region, stacks ] ) => regionMap[ region ] = stacks );
		return regionMap;
	} );
}
 ...
 */
function getRegion( config, stack ) {
	return getStacks( config )
		.then( map => {
			const region = Object.keys( map ).find( region => map[ region ].indexOf( stack ) >= 0 );
			if ( ! region ) {
				throw new Error( `Unable to find ${ stack } in any region` );
			}
